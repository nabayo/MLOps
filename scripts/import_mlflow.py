...
def import_mlflow_data(
    ...
    try:
        ...
        print(f"📊 Contains: {metadata['total_experiments']} experiments, "
        ...
        existing_experiments = {exp.name: exp for exp in client.search_experiments(view_type=3)}  # ALL
        ...
        for exp_data in experiments_data:
            ...
            if exp_name in existing_experiments:
                if skip_existing:
                    ...
                    experiment_id_mapping[exp_data['experiment_id']] = existing_experiments[exp_name].experiment_id
        ...
        for run_dir in sorted(runs_dir.iterdir()):
            ...
            if not dry_run:
                ...
                if artifacts_dir.exists():
                    for root, dirs, files in os.walk(artifacts_dir):
                        for file in files:
                            ...
                            artifact_path = str(rel_path.parent) if rel_path.parent != Path('.') else ""

                            try:
                                client.log_artifact(run.info.run_id, str(file_path), artifact_path)
                ...
                print(f"  ✓ Created: {run_name} ({stats['artifacts_uploaded']} artifacts)")
            else:
        ...
        if models_file.exists():
            ...
            # Get existing models
            existing_models = {model.name: model for model in client.search_registered_models()}
                ...
                for version_data in model_data.get('versions', []):
                    ...
                    if not dry_run and run_id:
                        try:
                            ...
                            print(f"    ⚠ Warning: Could not create version {version}: {e}")
                            stats['model_versions_skipped'] += 1
                    else:
        ...
        print("=" * 70)
        print(f"📦 Experiments: {stats['experiments_created']} created, {stats['experiments_skipped']} skipped")
        print(f"🏃 Runs: {stats['runs_created']} created, {stats['runs_skipped']} skipped")
        print(f"🎯 Models: {stats['models_created']} created")
        print(f"📦 Model Versions: {stats['model_versions_created']} created, {stats['model_versions_skipped']} skipped")
        print(f"📁 Artifacts: {stats['artifacts_uploaded']} uploaded, {stats['artifacts_skipped']} skipped")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Import MLflow data from a backup zip file")
    parser.add_argument("backup_file", help="Path to backup zip file")
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing data (default: skip)")
