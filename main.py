...
def run_training(args: argparse.Namespace) -> None:
    ...
    data_prep = DataPreparation(
        dataset_path=str(dataset_path), config=config, training_config=training_config
    )
...
def main():
    ...
    eval_parser.add_argument(
        "--model", type=str, required=True, help="Path to model weights (.pt file)"
    )
    eval_parser.add_argument(
        "--training-config",
        type=str,
    ...
    eval_parser.add_argument(
        "--run-id", type=str, default=None, help="MLflow run ID to log results to"
    )

    # Serve mode (handled by separate serving/api.py)
    _serve_parser = subparsers.add_parser(
        "serve", help="Start serving API (use serving/api.py instead)"
    )
    ...
    if args.mode == "train":
        ...
        print(
            "❌ Use 'python serving/api.py' or 'docker-compose up serving' to start the serving API"
        )
        sys.exit(1)
